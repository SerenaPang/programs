...
/**
 * Implementation of the operations for a min heap(priority queue) In a complete
 * binary tree, the relation of parent and child nodes can be well transffered
 * to the relation between two indexes - The root of the tree is at index 0 -
 * index of parent = i - parent of index = (i - 1) / 2 - left child of index = 2
 * * i + 1 - right child of index = 2 * i + 2
 */
public class MinHeap {
	...
	/**
	 * To make a tree maintain the heap's property By for each node that has at
	 * least one child(the parent nodes), we do a percolateDown in the order of from
	 * the nodes on the deepest level to the root
	 */
	 ...
	 * By compare the element with its parent, move it up when necessary. Do this
	 ...
	 * By comparing the elements with its 2 children, if the smallest one of the two
	 * children is smaller than the element, swap the element with that child. Do
	 * this ubtil the element does not need be moved.
	 */
	private void percolateDown(int index) {
		...
		while (index <= size / 2 - 1) { // last parent node, we don't want to review more than this
			...
			// choose left to be swap candidate, because it must have left child but not
			// necessary right child
			int swap = leftChild;
	...
	/**
	 * remove the first element in the min heap perform percolate down to maintain
	 * the heap's property
	 */
	public int poll() {
		if (size == 0) {
	...
	/**
	 * add an element to the min heap perform percolate up to maintain the heap's
	 * property
	 */
	...
	/**
	 * update the element in the min heap, either perform percolate up/down to
	 * maintain the heap's property
	 * 
